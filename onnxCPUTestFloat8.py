...
try:
    ...
    # ort_session = ort.InferenceSession('model/bestFloat8.onnx') # 양자화 :: 속도 개선 o, 정확도 떨어짐
    ort_session = ort.InferenceSession('model/bestFloat8-infer.onnx') # 양자화 > 최적화 :: 파일크기 o, 속도 o, 정확도 동일
    # ort_session = ort.InferenceSession('model/bestFloat8.quant.save_tensors.onnx')  # float32 > 최저확 > 양자화 > 디버깅 > 오류
    ...
    image = cv2.imread('images/ok3.jpg')
    ...
    # 이미지 정규화
    input_image = resized.astype(np.float32) / 255.0 # 픽셀 값을 0과 1 사이로 정규화
    input_image = input_image.transpose(2,0,1) # 이미지 차원을 변경. (높이, 너비, 채널) -> (채널, 높이, 너비)
    input_image = input_image[np.newaxis, :,:,:] # 입력 텐서 생성 및 타입 변경
