...
try:
    ...
    # ort_session = ort.InferenceSession('model/bestFloat16IOFix.onnx') # 입출력텐서 32float 고정(React-native 전용)
    # ort_session = ort.InferenceSession('model/bestFloat16.onnx') # 반양자화
    ort_session = ort.InferenceSession('model/bestFloat16-infer.onnx') # 반양자화 > 전처리 # 제일 빠르다.
    ...
    input_tensor = input_image[np.newaxis, :,:,:].astype(np.float16) # 입력 텐서 생성 및 타입 변경
